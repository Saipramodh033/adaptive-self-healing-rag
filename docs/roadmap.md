...
|---|---|---|
| **Hybrid Search (BM25 + dense vector)** | Retriever Recall@5 = 89.3% — keyword-heavy queries (exact product names, city names) miss exact noun matches in pure vector search | Close the remaining 10.7% recall gap; most direct path to higher Helpfulness |
...
| **Upgrade embedding model** | `bge-small-en-v1.5` is 384-dim — misses semantic nuance in complex, policy-heavy questions | Higher Recall@5 with zero infrastructure changes; drop-in replacement |
